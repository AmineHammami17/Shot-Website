...
exports.sendOrderConfirmation = async (userEmail, orderData) => {
    ...
    // Avoid hard-crashes when the logo path doesn't exist on this machine/container.
    const LOGO_PATH = process.env.EMAIL_LOGO_PATH || 'C:/Users/MARIEM/Desktop/shot/png_1.png';
...
exports.sendOrderStatusUpdate = async (userEmail, { orderId, numeroDeSuivi, statut }) => {
    const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: { 
            user: process.env.EMAIL_USER, 
    ...
    const statusLabel = String(statut || '').toUpperCase();

    const mailOptions = {
        from: '"S.HOT Shop" <noreply@shot.tn>',
        to: userEmail,
        subject: `Order Update: ${statusLabel} (#${String(orderId).slice(-6)})`,
        html: `
        <div style="font-family:Arial,sans-serif;background:#f4f7f9;padding:30px;">
          <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;padding:26px;border:1px solid #eef2f4;">
            <img src="cid:logo_shot" alt="S.HOT" width="90" style="display:block;margin:0 auto 18px;" />
            <h2 style="margin:0 0 8px;color:#0f172a;text-align:center;">Order status updated</h2>
            <p style="margin:0 0 18px;color:#475569;text-align:center;">
              Your order status is now <strong>${statusLabel}</strong>.
            </p>
            <div style="background:#ecfbf9;border:1px solid #d1fae5;border-radius:12px;padding:14px;color:#064e3b;">
              <div><strong>Order:</strong> #${String(orderId).slice(-8).toUpperCase()}</div>
              ${numeroDeSuivi ? `<div><strong>Tracking:</strong> ${numeroDeSuivi}</div>` : ''}
            </div>
            <p style="margin:18px 0 0;color:#64748b;font-size:12px;text-align:center;">© 2026 SHOT</p>
          </div>
        </div>`,
        attachments
    };
