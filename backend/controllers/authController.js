...
exports.register = async (req, res) => {
    try {
        ...
        try {
            ...
            // Keep registration successful even if SMTP/attachment fails in local environments.
            console.error("Erreur envoi email d'inscription:", emailError.message);
            const response = { message: "Utilisateur créé, mais l'email OTP n'a pas pu être envoyé." };
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        try {
            ...
            const response = { message: "Code généré, mais l'email n'a pas pu être envoyé." };
            if (process.env.NODE_ENV !== 'production') {
                response.devOtp = resetCode;
            }
            res.json(response);
        }
    } catch (error) {
