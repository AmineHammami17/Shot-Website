  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[500px] bg-white rounded-[40px] p-10 shadow-sm flex flex-col items-center border border-gray-100 text-center">
        ...
        {errorMessage && (
          <p className="text-sm font-medium text-red-500 mb-4">{errorMessage}</p>
        )}

        <button
          type="button"
          title="Continue"
          className="w-full mb-8 bg-[#238d7b] hover:bg-[#1db096] text-white font-bold py-4 rounded-3xl shadow-lg transition-all flex items-center justify-center gap-1 relative overflow-hidden"
        ...
        >
            <span className={`transition-all duration-300 ${isHoveredContinue ? '-translate-x-1' : 'translate-x-0'}`}>
              {isSubmitting ? 'Loading...' : t('btn_continue')}
            </span>
